...
import { useExternalProposals } from "@/hooks/useExternalProposals";
import {
...
import { ExternalProposalCard } from "@/components/senate/ExternalProposalCard";
...
// ──────────────────────────────────────────────
// Signaling Proposals (existing functionality)
// ──────────────────────────────────────────────
...
function ProposalCard({
  ...
  const collectionName =
    collection?.name ?? truncateAddress(proposal.nftContract);
    ...
    <div className="rounded-xl bg-gray-900 border border-gray-800 p-6">
      ...
      <div className="mt-4 pt-4 border-t border-gray-800">
        <div className="flex items-center justify-between text-sm mb-2">
          <span className="text-green-400 font-medium">Yes: {yesCount}</span>
          <span className="text-red-400 font-medium">No: {noCount}</span>
        </div>
        {totalVotes > 0 && (
...
function DaoProposalsTab({ isSenator }: { isSenator: boolean }) {
  const [daoFilter, setDaoFilter] = useState<string>("all");
  const [stateFilter, setStateFilter] = useState<string>("all");

  const filters: { dao?: string; state?: string } = {};
  if (daoFilter !== "all") filters.dao = daoFilter;
  if (stateFilter !== "all") filters.state = stateFilter;

  const { data: proposals, isLoading, error } = useExternalProposals(
    ...
    <div className="space-y-6">
      ...
      {isLoading && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 mt-4">Fetching DAO proposals...</p>
        </div>
      )}
      ...
      {error && (
        <div className="p-4 rounded-lg bg-red-900/20 border border-red-800">
          <p className="text-red-300 text-sm">
            Failed to load proposals. The Tally API key may not be configured.
          </p>
        </div>
      )}
      ...
      {!isLoading && !error && proposals && proposals.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-600 text-sm mt-2">
            {daoFilter !== "all" || stateFilter !== "all"
              ? "Try adjusting your filters."
              : "No active proposals from represented DAOs right now."}
          </p>
        </div>
      )}
      ...
      {!isLoading && proposals && proposals.length > 0 && (
        <div className="space-y-4">
          {proposals.map((proposal) => (
            <ExternalProposalCard
              key={`${proposal.daoSymbol}-${proposal.proposalId}`}
              proposal={proposal}
              isSenator={isSenator}
            />
          ))}
...
// ──────────────────────────────────────────────
// Signaling Proposals Tab
// ──────────────────────────────────────────────

function SignalingProposalsTab({
  isSenator,
  isConnected,
}: {
  isSenator: boolean;
...
type Tab = "dao" | "signaling";

export default function ProposalsPage() {
  const { address, isConnected } = useAccount();
  const isSenator = useIsSenator(address);
  const [activeTab, setActiveTab] = useState<Tab>("dao");
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-2">
        ...
        <p className="text-gray-400 mt-1">
          Vote on DAO proposals using the Meta Senate&apos;s delegated voting power,
          or create signaling proposals for the community.
        </p>
      </div>

      {isSenator && (
        <div className="mt-4 mb-6 p-4 rounded-lg bg-indigo-900/30 border border-indigo-700">
          <p className="text-indigo-300 text-sm">
            You are a Meta Senate senator. You can vote on DAO proposals and
            signaling proposals below.
          </p>
        </div>
      )}

      {/* Tabs */}
      <div className="flex gap-1 mt-6 mb-8 p-1 bg-gray-900 rounded-lg w-fit">
        <button
      ...
      {activeTab === "dao" && <DaoProposalsTab isSenator={isSenator} />}
      {activeTab === "signaling" && (
        <SignalingProposalsTab
          isSenator={isSenator}
